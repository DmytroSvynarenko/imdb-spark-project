...
def main():
    ...
    names_df = spark_session.read.csv('C:/Users/Dima/PycharmProjects/imdb-spark-project/Data/name.basics.tsv.gz',
                                      sep="\t", header=True, nullValue='Null', schema=names_schema).show(30, truncate=False)
...
#    title_2_hours_df = spark_session.read.csv('C:/Users/Dima/PycharmProjects/imdb-spark-project/Data/title.basics.tsv.gz',
...
#    title_2_hours_df = title_2_hours_df.withColumn('runtime_hours', title_2_hours_df.runtime_hours.cast(t.IntegerType()))
#    title_2_hours_df.filter((f.col('runtime_hours') != 0) & (f.col('title_type') == 'movie')).select('title_type', 'primary_title', 'original_title',
#            'runtime_minutes').write.csv('C:/Users/Dima/PycharmProjects/imdb-spark-project/Data/3', header=True, mode='overwrite')
    ...
    names_characters_df = spark_session.read.csv('C:/Users/Dima/PycharmProjects/imdb-spark-project/Data/title.principals.tsv.gz',
                                      sep="\t", header=True, nullValue='Null', schema=names_characters_schema)
    names_characters_df.sort(f.col('nconst')).show()
#    names_characters_df.groupBy('nconst').count().orderby('nconst', ascending=True).show()
